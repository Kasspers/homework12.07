...
func main() {
	...
	authorsApi.GET("/show", allAuthors)
	authorsApi.POST("/create", createAuthor)
	authorsApi.POST("/delete",deleteAuthor)
	authorsApi.POST("/update", updateAuthor)

	genreApi := r.Group("api/genres")
	genreApi.GET("/show", allGenres)
	genreApi.POST("/create", createGenre)
	genreApi.POST("/delete", deleteGenre)
	genreApi.POST("/update", updateGenre)

	readerApi := r.Group("api/readers")
	readerApi.GET("/show", allReaders)
	readerApi.POST("/create", createReader)
	readerApi.POST("/delete", deleteReader)
	readerApi.POST("/update", updateReader)

	bookApi := r.Group("api/books")
	bookApi.POST("/show", showBooks)
	bookApi.POST("/create", createBook)
	bookApi.POST("/delete", deleteBook)
	bookApi.POST("/update", updateBook)
...
func rentABook(c *gin.Context) {
	...
	err := c.Bind(&book)
	_, err = db.QueryOne(book, `UPDATE book SET current_reader = (?current_reader) WHERE book_id = (?book_id) RETURNING *` , book)
	if err == nil{
		c.String(200, fmt.Sprint(book.CurrentReader," арендовал ",book.Name))
		...
		if err == nil{
			c.String(200, fmt.Sprint(history,"Аренда записана"))
			return
		}
		c.JSON(400, gin.H{
			"error msg": err.Error(),
...
func allGenres(c *gin.Context) {
	...
	_, err = db.Query(&genre, `SELECT * FROM genre`)
	if err == nil{
		c.JSON(400, gin.H{
			"result":genre,
		})
...
(SELECT 1 FROM book b WHERE a.author_id = b.author_id AND a.author_id = ?) RETURNING *` , authorID.AuthorId,authorID.AuthorId)
	if err.Error() == "pg: no rows in result set"{
		c.String(200, fmt.Sprint("Такого автора не существует/Нельзя удалить автора с существующими книгами"))
		return}
	if err == nil{
